...
# socket options for reusing address
...
def player_collision(players):
	...
	for x, player1 in enumerate(sort_players):
		for player2 in sort_players[x+1:]:
			...
			if (dis < players[player2]["score"] - players[player1]["score"]*0.85) and not (players[player2]["score"] == players[player1]["score"]):
...
def create_balls(balls, n):
	...
	for i in range(n):
		...
		# this loop prevents ball spawning on top of player
		while True:
			stop = True
...
def threaded_client(conn, _id):
	...
	connections -= 1 
	del players[current_id]  # remove client information from players dictionary
